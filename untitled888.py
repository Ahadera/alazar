...
# Attempt to install yfinance if it's not available
try:
    import yfinance as yf
except ModuleNotFoundError:
    subprocess.check_call([sys.executable, "-m", "pip", "install", "yfinance"])
    import yfinance as yf  # Retry importing after installation
...
class StockAnalyzer:
    ...
    def fetch_data(self):
        """Download stock data for the specified stocks."""
    ...
    def analyze_drops(self):
        """Analyze significant drops in stock prices."""
        ...
        for stock, data in self.data_dict.items():
            ...
            if not significant_drops.empty:
                ...
                for date in significant_drops.index:
                    drops.append((date, significant_drops.loc[date, 'Quarterly Change'] * 100))
    ...
    def calculate_total_value(self, stock_data):
        ...
        for stock, details in self.stocks.items():
            if stock in stock_data:
                current_price = stock_data[stock]['Close'].iloc[-1]  # Latest closing price
...
if st.sidebar.button("Fetch Data and Analyze"):
    ...
    if analyzer.data_dict:
        ...
        if portfolio_input and purchase_price_input:
            portfolio = Portfolio()
            stocks_with_shares = portfolio_input.split(',')
            purchase_prices = purchase_price_input.split(',')

            for stock_with_shares, price in zip(stocks_with_shares, purchase_prices):
                stock_symbol, shares = stock_with_shares.split(':')
            ...
            st.write(f"Total Portfolio Value: ${portfolio.calculate_total_value(analyzer.data_dict):.2f}")
